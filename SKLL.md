...
description: >
  ...
  Analyze OBV, A/D Line, CMF, and VWAP volume indicators for stock OHLCV data.

  Use whenever user mentions volume indicators, money flow, accumulation/distribution,

  trading volume analysis, OBV, A/D, CMF, VWAP, or asks about "量能分析"/"资金流向"/"成交量指标".

  Generates a structured report with trends, divergences, intensity ratings, and composite
...
(这里写你后续要补充的具体执行步骤，目前可以先留空或写简略版)

1. **Validate and prepare data** – ensure at least 30 trading days.

2. **Compute OBV** – detect divergence.

3. **Compute A/D Line** – compare with OBV.

4. **Compute CMF(20)** – assign strength rating.

5. **Compute VWAP** – compare close price.

6. **Synthesize composite conclusion** – generate trade suggestions.
